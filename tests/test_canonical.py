...
def test_import_fills_qian_kun(tmp_path):
    ...
    assert stats["canonical_filled"] == 384    # all 64 hexagrams x 6 lines
    assert stats["hexagrams_covered"] == 64
    ...
    assert by[("乾", "初九")]["canonical_text"] == "潛龍，勿用。"
    assert by[("坤", "上六")]["canonical_text"] == "龍戰于野，其血玄黃。"
    assert by[("屯", "初九")]["canonical_text"] == "磐桓；利居貞，利建侯。"
    assert by[("坤", "六二")]["action"] == "jin"          # P0 override applied
    assert by[("屯", "初九")]["modern_text"] == ""        # no P1 text for 屯 yet
